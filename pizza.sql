...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetCustomersByRatingAndFood` (IN `p_rating` VARCHAR(3), IN `p_food_id` INT)   BEGIN
    -- Kiểm tra giá trị x sao có hợp lệ không (1 đến 5 hoặc 'all')
    IF p_rating NOT IN ('1', '2', '3', '4', '5', 'all') THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Số sao nhập vào phải trong khoảng từ 1 đến 5 hoặc "all"';
    END IF;
    ...
    ELSE
        ...
        INNER JOIN 
            user_review ur ON c.customer_id = ur.customer_id
        WHERE 
            ur.rating = p_rating AND ur.food_id = p_food_id;
    END IF;
END$$
...
('https://images.pexels.com/photos/27583268/pexels-photo-27583268/free-photo-of-a-pizza-with-a-slice-missing-from-it-on-a-wooden-board.jpeg?auto=compress&cs=tinysrgb&w=600&lazy=load.jpg\r\n', 8),
...
(4, 6, 1, 4, NULL, '321 Maple Ave, Nowhere USA', 200000, '2023-08-05'),
(5, 7, 2, 5, NULL, '654 Pine Rd, Everytown USA', 120000, '2023-09-10'),
(6, 8, 0, 6, NULL, '987 Birch Blvd, Anotherton USA', 90000, '2023-10-01'),
(7, 9, 1, 7, NULL, '159 Oak St, Somewhereville USA', 180000, '2023-11-15'),
...
(16, 5, 1, '4', 'Pizza hải sản rất ngon, đầy đủ các loại hải sản tươi ngon.'),
(17, 6, 1, '3', 'Pizza hải sản tạm được, không đủ các loại hải sản .'),
(18, 7, 1, '2', 'Tạm được.');
