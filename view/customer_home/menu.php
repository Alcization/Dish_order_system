...
<body>
    ...
    // Đếm tổng số món ăn
    $total_query = "SELECT COUNT(*) as total FROM food";
        ...
        <div class="row">
            <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                <div class="col-md-3 mb-4">
                    <div class="card">
                        <!-- Hình ảnh -->
                        <img src="<?= htmlspecialchars($row['food_image_url'] ?: '../image/logo.jpg') ?>"
                            class="card-img-top" alt="<?= htmlspecialchars($row['food_name']) ?>">

                        <div class="card-body">
                            <!-- Tên món ăn -->
                            <!-- Nút thêm món -->
                            <form method="POST" action="" onsubmit="showNotification(event, this)">
                                <h5 class="card-title" value="<?= htmlspecialchars($row['food_name']) ?>">
                                    <?= htmlspecialchars($row['food_name']) ?></h5>
                                ...
                                <span class="notification-message"
                                    style="display: none; margin-left: 10px; color: green;">Đã thêm món!</span>
                            </form>
                        </div>
                    </div>
                </div>
